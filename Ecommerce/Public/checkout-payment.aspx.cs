    ...
    public partial class checkout_payment : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["UserID"] != null && Session["UserName"] != null)
            {
                ...
                {
                    lbl_AraToplam.InnerText = "0.00 TL";
                    lbl_genelToplam.InnerText = "0.00 TL";
                    LiteralCheckoutItems.Text = "<div class=\"row mx-0 py-4 g-0 border-bottom\">" +
        ...
        private void ListCartItems(List<Dal.Cart> cart)
        {
            ...
            foreach (var cartItem in GroupedCart)
            {
                ...
                {
                    LiteralCheckoutItems.Text += "<p class=\"fw-bolder text-end text-muted m-0\">" + urun.UrunIndirimFiyat + " TL</p></div></div>";
                    total += Convert.ToInt32(urun.UrunIndirimFiyat) * Quantity;
                }
                else
                {
                    LiteralCheckoutItems.Text += "<p class=\"fw-bolder text-end text-muted m-0\">" + urun.UrunFiyat + " TL</p></div></div>";
                    total += Convert.ToInt32(urun.UrunFiyat) * Quantity;
                }
            }

            lbl_AraToplam.InnerText = total.ToString() + ".00 TL";
            lbl_genelToplam.InnerText = total + 25 + ".00 TL";
        }
    }
}